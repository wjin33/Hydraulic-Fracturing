...
%                 edgelength = sqrt((edge_coordinates(2,2)-edge_coordinates(1,2))^2 + (edge_coordinates(2,3)-edge_coordinates(1,3))^2);
                ind = [ind; NODES(edge_coordinates(1,1),2); NODES(edge_coordinates(2,1),2)];            
                globalF(3*edge_coordinates(1,1),1) = globalF(3*edge_coordinates(1,1),1) + injection_rate;   % Share of node from Elemental flux flow into the domain                    des
                globalF(3*edge_coordinates(2,1),1) = globalF(3*edge_coordinates(2,1),1) + injection_rate; 
            end
            break;
        end
    end
...
EXTFlUX(:,1) = index;
m=nnz(globalF);
EXTFlUX(:,2) = injection_rate/m*ones(m,1);

ind = unique(ind);
for i=1:size(ind,1)
%     EXTDISP = [EXTDISP; ind(i)*3-2, 0; ind(i)*3-1, 0;];
    EXTPRESSURE = [EXTPRESSURE;ind(i)*3, 0;];
end

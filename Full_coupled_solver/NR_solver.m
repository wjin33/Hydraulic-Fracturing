...
%%
global DISPDD DISPTD DISPTD_Rate FORCE GKF PREFTD NODES

%Initialize the field variables and Determine the total number of degree of freedom
NEQ = 3*max(max(NODES));          % Total number of degrees of freedom
PREFTD = zeros(NEQ,1);
DISPTD_Rate= zeros(NEQ,1);
DISPTD = PREFTD;
DISPDD = sparse(NEQ,1);
...
%   DISPTD_Rate = 1/(Theta*deltaT).*DISPDD-(1/Theta-1).*DISPTD_Rate;
  %
  ...
  if (ISTEP  >= 1)
      ...
      while (Propagation)
        ...
        if Propagation
            ...
            DISPTD = TEMP;
%           TEMP  = zeros(NEQ,1);
%           TEMP(1:size(DISPTD_Rate,1),1) = DISPTD_Rate;
%           DISPTD_Rate = TEMP;
...
%     SDISP = deltaT*EXTDISP(:,2)/TDELTA*(CUR2-CUR1);
    ...
    while(FLAG30 == 1)
      ...
      if size(EXTFlUX,1)>0
         f_index = EXTFlUX(:,1);
         FORCE(f_index,1) = FORCE(f_index,1) + deltaT*EXTFlUX(:,2);
      end
...
%       toc
      ...
      % Check convergence
      if(ITER>1)
        KNOWNDOF =  [EXTDISP(:,1); EXTPRESSURE(:,1)];       % find degree of freedom with applied boundary condition
        ...
        OUTPUT(1, ITER, RESN, TIME, deltaT)
        
%         FLAG10 = 1;
%           break;
        
        %
        if(RESN<TOL || (React_tot<TOL && Residul_tot<TOL))
          FLAG10 = 1;
          break;
        end
        %
      ...
      if NPRE~=0
        ...
        GKF(FIXEDDOF,:)=zeros(NPRE,NEQ);
        GKF(FIXEDDOF,FIXEDDOF)=1E15*eye(NPRE);
        FORCE(FIXEDDOF)=0;
        if (ITER==1), FORCE(FIXEDDOF) = 1E15*EXTPRESSURE(:,2); end
      end
      
      % Solve the system equation
      if(FLAG20 == 0)
        SOLN = GKF\FORCE;
        ind = find(isnan(SOLN)>0);
        SOLN(ind,1) = zeros(size(ind));
        DISPDD = DISPDD + SOLN;
        DISPTD = DISPTD + SOLN;
        FLAG30 = 1;
      else
...
function OUTPUT(FLG, ITER, RESN, TIME, deltaT)
%*************************************************************************
...
%%
  if FLG == 1
    if ITER>2
      ...
      fprintf(1,'\n	Time   TimeInc    Iter	 Residual \n');
      fprintf(1,'%10.5f %10.3e %5d %14.5e \n',TIME,deltaT,ITER,full(RESN));
    end
  end
end

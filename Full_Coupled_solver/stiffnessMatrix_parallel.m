...
[Rhs, StiffnessMatrix]=parallel_for(CONNEC, NODES, PSI, XYZ, STATEV, DISPDD, DISPTD, PREFTD, FORCE, GKF,  connec_frac, xyz_frac, statev_frac,PROP,LTAN,NLTAN,Theta,deltaT,NEQ);
...
function [Rhs, StiffnessMatrix]=parallel_for(CONNEC, NODES, PSI, XYZ, STATEV, DISPDD, DISPTD, PREFTD, FORCE, GKF, connec_frac, xyz_frac, statev_frac,PROP,LTAN,NLTAN,Theta,deltaT,NEQ)

StiffnessMatrix = GKF;
Rhs = FORCE;
fcn = @plus;

NumbElement= size(CONNEC,1);

parfor iElem = 1:NumbElement
...
%                       (1-xi)*(1-eta) (1+xi)*(1-eta)];
            ...
            [NLEquivStrain,scale] = computeNonlocalEquivalentStrain( nonlocal_table,STATEV );
            ...
            Felement =  - W*detJ*Bu'*[stressN(1:2,1); stressN(4,1)];
            Rhs = fcn(Rhs,sparse(localD,1,Felement,NEQ,1));
            ...
            Felement =  - QT*(DISPTD(localD)-PREFTD(localD)) - M*(DISPTD(localP)-PREFTD(localP))...
                            -deltaT*H*(Theta*DISPTD(localP)+(1-Theta)*PREFTD(localP)) ;
            Rhs = fcn(Rhs,sparse(localP,1,Felement,NEQ,1));

            %Update tangent/secant stiffness
            if LTAN
                C=Gauss_tan_update(PROP,damageN);                                        % Damage dependent stiffness matrix
                Kelement = W*Bu'*C*Bu*detJ;
                mapRow=reshape(localD'*ones(1,size(Kelement,2)),[],1);
                mapCol = reshape(ones(size(Kelement,1),1)*localD,[],1);
                StiffnessMatrix = fcn(StiffnessMatrix,sparse(mapRow,mapCol,reshape(Kelement,[],1),NEQ,NEQ));
                Kelement = M + H*(Theta*deltaT);
                mapRow=reshape(localP'*ones(1,size(Kelement,2)),[],1);
                mapCol = reshape(ones(size(Kelement,1),1)*localP,[],1);
                StiffnessMatrix = fcn(StiffnessMatrix,sparse(mapRow,mapCol,reshape(Kelement,[],1),NEQ,NEQ));
                Kelement = - Q;
                mapRow=reshape(localD'*ones(1,size(Kelement,2)),[],1);
                mapCol = reshape(ones(size(Kelement,1),1)*localP,[],1);
...
%             end
            ...
            if (histo_jump(1)==1e6)
                ...
                Traction =  cohesiveLaw(jump, histo_jump,PROP);
            end
...
%             end
            ...
            Felement = - W*detJ_frac*NT'*lambda*Traction + W*detJ_frac*NT'*norm_frac'*Nen*DISPTD(localP);
            Rhs = fcn(Rhs,sparse(localD_enr,1,Felement,NEQ,1));                                                        
%             FORCE(localD_une) = FORCE(localD_une) - W*detJ_frac*NT'*norm_frac'*Nen*DISPTD(localP);                                     
            Felement = - L_upT*(DISPTD(localD_enr)-PREFTD(localD_enr))...
...
%                                           - W*detJ_frac*Nen'*(norm_frac*( 1/(Theta*deltaT)*NT*DISPTD(localD_enr)))...
            ...
            if LTAN
                if (histo_jump(1)==1e6)
                    ...
                    M_czm =  cohesiveStiffness(jump, histo_jump,PROP);
                end
                K_CZM = W*NT'*lambda'*M_czm*lambda*NT*detJ_frac;  %% cohesive zone section
                
                mapRow=reshape(localD_enr'*ones(1,size(K_CZM,2)),[],1);
                mapCol =reshape(ones(size(K_CZM,1),1)*localD_enr,[],1);
                StiffnessMatrix = fcn(StiffnessMatrix,sparse(mapRow,mapCol,reshape(K_CZM,[],1),NEQ,NEQ));
                Kelement = - L_up;
                mapRow  =  reshape(localD_enr'*ones(1,size(Kelement,2)),[],1);
                mapCol  =  reshape(ones(size(Kelement,1),1)*localP,[],1);
                StiffnessMatrix = fcn(StiffnessMatrix,sparse(mapRow,mapCol,reshape(Kelement,[],1),NEQ,NEQ));
                Kelement = + L_upT;
                mapRow  =  reshape(localP'*ones(1,size(Kelement,2)),[],1);
                mapCol  =  reshape(ones(size(Kelement,1),1)*localD_enr,[],1);
                StiffnessMatrix = fcn(StiffnessMatrix,sparse(mapRow,mapCol,reshape(Kelement,[],1),NEQ,NEQ));
                Kelement = M_frac + (Theta*deltaT)*H_frac; 
                mapRow  =  reshape(localP'*ones(1,size(Kelement,2)),[],1);
                mapCol  =  reshape(ones(size(Kelement,1),1)*localP,[],1);
                StiffnessMatrix = fcn(StiffnessMatrix,sparse(mapRow,mapCol,reshape(Kelement,[],1),NEQ,NEQ));
            end
...
function [traction] =  cohesiveLaw(jump, histo_jump,PROP)
...
function [stiffness] =  cohesiveStiffness(jump, histo_jump,PROP)

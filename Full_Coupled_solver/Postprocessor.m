...
function Postprocessor(STEP,TIME,file_name)
% This function output the all the primary and secondary results of the
% simulation into vtk formate for post processing

global  PHI PSI XYZ CONNEC NODES STATEV DISPTD connec_frac xyz_frac statev_frac
...
filename = strcat(file_name,'_PP_Jump.txt');
fid = fopen(filename, 'a');
fprintf(fid, '\n\n # %d  %8.6e\n',STEP, TIME); 
for i = 1:NELE
    for j = 2:-1:1
        fprintf(fid,'  %8.6e   %8.6e   %8.6e \n', statev_frac{i}{j}.true_coodinates(1), statev_frac{i}{j}.porepressure,statev_frac{i}{j}.width);
    end
end
fprintf(fid, '\n');
fclose(fid);

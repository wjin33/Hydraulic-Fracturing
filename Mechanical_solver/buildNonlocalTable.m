...
function  buildNonlocalTable(PROP,BElset)
% This function is try to build the nonlocal gauss point table 

 tic

 global CONNEC STATEV XYZ CRACK PSI LocalElement

 LocalElement = BElset{1,2};
  ...
  for iElem = 1:NE
      ind=find(LocalElement==iElem);
      if nnz(ind)
          STATEV{iElem}{1}.nonlocalTable = [iElem 1 1];
          STATEV{iElem}{2}.nonlocalTable = [iElem 2 1];
          STATEV{iElem}{3}.nonlocalTable = [iElem 3 1];
          STATEV{iElem}{4}.nonlocalTable = [iElem 4 1];
          continue
      end

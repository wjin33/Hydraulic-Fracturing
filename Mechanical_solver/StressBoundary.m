    ...
    for iSurf = 1:n
        if (strcmp(Bsurface{iForce,1},surf(iSurf))>0)
            ...
            for iedge = 1:num_edge
                ...
                globalF(2*edge_coordinates(1,1)-1,1) = globalF(2*edge_coordinates(1,1)-1,1) + force(1)/2;                      % Elemental force in x-direction for edge nodes
                globalF(2*edge_coordinates(1,1)-0,1) = globalF(2*edge_coordinates(1,1)-0,1) + force(2)/2;                      % Elemental force in y-direction for edge nodes
                globalF(2*edge_coordinates(2,1)-1,1) = globalF(2*edge_coordinates(2,1)-1,1) + force(1)/2;                      % Elemental force in x-direction for edge nodes
                globalF(2*edge_coordinates(2,1)-0,1) = globalF(2*edge_coordinates(2,1)-0,1) + force(2)/2;   
            end
%             break;
        end
    end

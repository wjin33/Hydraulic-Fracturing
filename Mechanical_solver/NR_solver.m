...
function NR_solver(CONTROL,PROP,EXTFORCE,EXTDISP,injection_rate)
%***********************************************************************
...
%     ISTEP = ISTEP - 1;				% Decrease load increment
  ...
  if (ISTEP>0)
      ...
      UPDATE=true; NLTAN=false; LTAN=true; 
...
%   if (ISTEP  >= 4)
      ...
      if Propagation
          ...
          StateV_update;
          NEQ=[0 0];
          NEQ(1) = 2*max(max(NODES));                         % Number of degrees of freedom for displacement
          NEQ(2) = size(xyz_frac,1);                  % Number of degrees of freedom for pressure
          TEMP  = zeros(NEQ(1),1);                              % The displacement as well as the pore pressure is initially zero;
          ...
          TEMP = HydroPressure(1);
          HydroPressure = TEMP*ones(NEQ(2),1);
...
%     SDISP = deltaT*EXTDISP(:,2)/TDELTA*(CUR2-CUR1);
    ...
    while(FLAG30 == 1)
      ...
      if( ITER> 1)
        ...
        %
        if ((RESN<TOL || ( Residul_tot<TOL)) && Pressure_error < TOL)
          FLAG10 = 1;
          break;
        end
        %
      ...
      if NDISP~=0
        FIXEDDOF=EXTDISP(:,1);
        GKF(FIXEDDOF,:)=zeros(NDISP,NEQ(1));
        GKF(FIXEDDOF,FIXEDDOF)=1E15*eye(NDISP);
        %
        FORCE(FIXEDDOF)=0;
        if ITER==1, FORCE(FIXEDDOF) = 1E15*EXTDISP(:,2); end
      end
      ...
      if(FLAG20 == 0)
        ...
        DISPTD = DISPTD + SOLN;
        FracVolume();
        Volumeinj = injection_rate*TIME;
        HydroPressure(1) = HydroPressure(1)*Volumeinj/Fracvolume;
        Pressure_error = abs(HydroPressure(1)- HydroPressure(2))/HydroPressure(2);
        HydroPressure = HydroPressure(1)*ones(size(HydroPressure,1),1);
        FLAG30 = 1;
      else
        FLAG30 = 0;
      end
